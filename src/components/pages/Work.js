import React from 'react';

export default function Work() {
    return(
        <div className="work" id="work">
            <h2>My Projects</h2>
            <div className="work-main">
                <a href="https://philipbrowning.github.io/project1/" className="large-card" id="card1">
                    <div className="label">
                      <h3>Recipe Planner</h3>
                      <a className="repo-link" href="https://github.com/PhilipBrowning/project1">Link to Repository</a>
                    </div>
                </a>

                <a href="https://lit-coast-69358.herokuapp.com/" className='large-card' id="card2">
                    <div className='label'>
                        <h3>Review Website</h3>
                        <a className='repo-link' href="https://github.com/zmoore371/Project-2">Link to Repository</a>
                    </div>
                </a>
    
                <a href="https://zmoore371.github.io/Homework-Week-6/" className="large-card" id="card3">
                    <div className="label">
                      <h3>Weather Dashboard</h3>
                      <a className="repo-link" href="https://github.com/zmoore371/Homework-Week-6">Link to Repository</a>
                    </div>
                </a>
                
                <a href="https://zmoore371.github.io/Homework-Week-5/" className="large-card" id="card4">
                    <div className="label">
                      <h3>Day Planner</h3>
                      <a className="repo-link" href="https://github.com/zmoore371/Homework-Week-5">Link to Repository</a>
                    </div>
                </a>
                
                <a href="https://github.com/zmoore371/Online-Offline-Budget-Tracker" className="large-card" id="card5">
                    <div className="label">
                      <h3>Budget Tracker PWA</h3>
                      <a className="repo-link" href="https://budget-tracker-9001.herokuapp.com/">Link to Repository</a>
                    </div>
                </a>
                
                <a href="https://github.com/zmoore371/Homework-Week-12-Employee-Tracker" className="large-card" id="card6">
                    <div className="label">
                      <h3>SQL Command Line</h3>
                      <a className="repo-link" href="https://github.com/zmoore371/Homework-Week-3">Link to Repository</a>
                    </div>
                </a>  
                
            </div>
        </div>
    )
}